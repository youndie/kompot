...
public data class KompotEventDescriptor(
...
public sealed class AnalyticsEvent {
    ...
    public data class ActionTracked(
        val descriptor: KompotEventDescriptor,
    ) : AnalyticsEvent()

    public data class ComponentImpression(
        val descriptor: KompotEventDescriptor,
    ) : AnalyticsEvent()
    ...
    public data class ExperimentExposure(
        val experimentId: String,
        val variantId: String,
    ) : AnalyticsEvent()

    public data class FieldChanged(
        val formId: String,
        val fieldId: String,
    ) : AnalyticsEvent()

    public data class FieldValidationErrorShown(
        val formId: String,
        val fieldId: String,
        val message: String,
    ) : AnalyticsEvent()

    public data class FormSubmitAttempted(
        val formId: String,
    ) : AnalyticsEvent()

    // getPayload() == null: at least one visible field failed validation.
    public data class FormSubmitBlocked(
        val formId: String,
    ) : AnalyticsEvent()
    ...
    public data class FormSubmitSucceeded(
        val formId: String,
    ) : AnalyticsEvent()
}
