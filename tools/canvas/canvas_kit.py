...
The markup contract (what `canvas_tree.py` reads):
    ...
    surface         data-tone  data-density  data-spacing  data-align="start|center"  data-pinned  data-weight  data-action
...
def build_css(tokens: dict, prefix: str) -> str:
    ...
    for name, e in tokens.get("type", {}).items():
        ...
        lines.append(f"[data-style={key}] {{ font-size: {e['size']}px; font-weight: {e['weight']}; line-height: {line}px; letter-spacing: {e.get('letterSpacing', 0)}px; }}")
...
def card(group: str, title: str, body: str, css: str, width: int = 1080) -> str:
    return (
        ...
        f"<link rel=\"preconnect\" href=\"https://fonts.googleapis.com\"><link href=\"https://fonts.googleapis.com/css2?family=Roboto+Flex:opsz,wght,wdth@8..144,100..1000,25..151&display=swap\" rel=\"stylesheet\">\n"
