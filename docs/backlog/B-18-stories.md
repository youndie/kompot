...
`stories/{Stories,ViddikStories}.kt`; в конфиге `samples` и `vocabulary`; в левой колонке — списки
«Stories» и «Screenshot fixtures», клик по истории открывает её как обычный экран, клик по фикстуре
рисует композицию во фрейме бренда. Шесть тестов в `StoriesTest`; окно проверено запуском.

- **Вариация — правка ОДНОГО свойства в закодированном JSON, а не пересборка компонента.** Пересборка
  означала бы `when` по каждому типу, который завёл потребитель, — ровно тот список, ради отказа от
  которого панель и существует.
- **Образец кодируется полиморфно** (`encodeKompotComponent`): конкретный сериализатор не пишет
  дискриминатор у корня, и история, которая никуда не декодируется, была бы картинкой того самого
  дефекта, ради которого превью принимает строку, а не объект.
- **Тип со словами и без образца показан как пробел, а не пропущен.** «У какого нашего компонента
  никто ни разу не нарисовал это состояние» — вопрос, на который панель отвечает только если пробелы
  на ней. Строка есть, кликом не открывается, помечена `(no sample)`.
- **Рефлексия к `GeneratedViddikRegistry` проверена против настоящего класса.** В тестовом сорссете
  стоит объект с тем самым FQN (`ru.workinprogress.viddik.generated.GeneratedViddikRegistry`) и той же
  формой — оба взяты **из процессора линии 0.1.1.8**, а не из более новой. Без этого покрытым остался
  бы только отсутствующий случай, а он проходит вообще без реализации.
- **Отсутствующего `ViddikShowroomLauncher` на этой линии нет** — ресёрч описывал его по более новой
  версии viddik. Имя генерируемого класса взято из байткода процессора, а не из того описания.
- Фикстура рисуется **внутри фрейма потребителя**: композиция вне бренда — картинка того, чего никто
  не выпускает.
