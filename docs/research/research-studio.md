...
- ~~`DecoratedWindow` рассчитан на JetBrains Runtime — тот же JBR, что нужен Compose Hot Reload;
  ...
  **Неверно, исправлено спайком B-08 (03.09.2026).** `DecoratedWindow` не деградирует: первая его
  ...
  (`JvmVendorSpec.JETBRAINS`) — лениво, только на запуске и на тестах.
- **Jewel 0.40 требует Java 25 в рантайме** (найдено B-09): собран из платформы IJ 262 и несёт
  **class file 69**. На JBR 21 студия падала `UnsupportedClassVersionError` на `JewelTheme` —
  после того, как всё скомпилировалось, опубликовалось и прошло тесты. Поэтому toolchain запуска —
  **JBR 25**. Публикуемый байткод при этом остаётся на пороге тулкита (17): версия JVM, на которой
  студию *запускают*, и версия, под которую её *компилируют*, — разные числа, и второе не двигалось.
